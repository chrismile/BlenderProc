...
class Front3DRefinedLoader:
    ...
    The Front3DRefinedLoader creates automatically lights in the scene, by adding emission shaders to the ceiling and
    lamps.
    """

    @staticmethod
    def load(
            json_path: str, front_texture_path: str, future_model_path: str, mapping: dict, model_id_category_dict: dict,
        ...
        :param front_texture_path: Path to the wall/floor textures used in the 3D-Front dataset.
        ...
        :param model_id_category_dict: A dict which maps the jids of objects to category names.
        ...
        front_texture_path = Utility.resolve_path(front_texture_path)
        ...
        if not os.path.exists(front_texture_path):
            raise Exception("The 3D front texture path does not exist: {}".format(front_texture_path))
        ...
        created_objects = Front3DRefinedLoader._create_mesh_objects_from_file(
            data, ceiling_light_strength, mapping, json_path, front_texture_path)

        all_loaded_furniture = Front3DRefinedLoader._load_furniture_objs(
            data, future_model_path, lamp_light_strength, mapping, model_id_category_dict)

        created_objects += Front3DRefinedLoader._move_and_duplicate_furniture(data, all_loaded_furniture)
    ...
    @staticmethod
    def _create_mesh_objects_from_file(
            data: dict, ceiling_light_strength: float, mapping: dict,
            json_path: str, front_texture_path: str) -> List[MeshObject]:
        """
        ...
        for mat in data["material"]:
            used_mat = {"uid": mat["uid"], "jid": mat["jid"],
                        "texture": mat["texture"], "normaltexture": mat["normaltexture"], "color": mat["color"]}
            if "useColor" in mat:
                used_mat["useColor"] = mat["useColor"]
            else:
                used_mat["useColor"] = not mat["texture"]
            used_materials.append(used_mat)
        ...
        for mesh_data in data["mesh"]:
            ...
            if used_mat:
                if used_mat["texture"] or used_mat["color"]:
                    ...
                    if used_mat["texture"]:
                        if "jid" not in used_mat:
                            print(used_mat)
                        texture_path = os.path.join(front_texture_path, used_mat["jid"], "texture.png")
                        MaterialLoaderUtility.add_base_color(mat.nodes, mat.links, texture_path, principled_node)
                    if used_mat["color"] and used_mat["useColor"]:
                    ...
                    if "ceiling" in used_obj_name.lower():
                        mat.make_emissive(
                            ceiling_light_strength, keep_using_base_color=False,
    ...
    def _load_furniture_objs(
            data: dict, future_model_path: str, lamp_light_strength: float, mapping: dict,
            model_id_category_dict: dict) -> List[MeshObject]:
        """
